...
if (!class_exists('gsShortCodeInMenu')) {
    ...
    class gsShortCodeInMenu {
        ...
        public function setup_meta_box() {
            ...
            add_meta_box('add-shortcode-section', __('Callback Function'), array($this, 'meta_box2'), 'nav-menus', 'side', 'default');
        }
        ...
        public function meta_box() {
            ...
            <div class="gs-sim-div" id="gs-sim-div">
                ...
                <p id="menu-item-title-wrap">
                    ...
                    <input id="gs-sim-title" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-title]" type="text" class="regular-text menu-item-textbox" title="<?php esc_attr_e('Title'); ?>" style="width:100%" value="Menu item title"/>    
                </p>

                <p id="menu-item-html-wrap">
                    <textarea style="width:100%;" rows="9" id="gs-sim-html" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-description]" class="code menu-item-textbox" title="<?php esc_attr_e('Text/html/shortcode here!'); ?>">[gs_test_shortcode]</textarea>
                </p>
        ...
        public function meta_box2() {
            ...
            $object_id = $this->new_object_id($last_object_id);
            ?>
            <div class="gs-cim-div" id="gs-cim-div">
                <input type="hidden" class="menu-item-db-id" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-db-id]" value="0" />
                <input type="hidden" class="menu-item-object-id" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-object-id]" value="<?php echo $object_id; ?>" />
                <input type="hidden" class="menu-item-object" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-object]" value="gs_cim" />
                <input type="hidden" class="menu-item-type" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-type]" value="" />
                <input type="hidden" id="gs-sim-description-nonce" value="<?php echo wp_create_nonce('gs-cim-description-nonce') ?>" />
                <p id="menu-item-title-wrap">
                    <label for="gs-cim-title"><?php _e('Title'); ?></label>
                    <input id="gs-cim-title" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-title]" type="text" class="regular-text menu-item-textbox" title="<?php esc_attr_e('Title'); ?>" style="width:100%" value="Menu item title"/>    
                </p>

                <p id="menu-item-html-wrap">
                    <textarea style="width:100%;" rows="9" id="gs-cim-html" name="menu-item[<?php echo $_nav_menu_placeholder; ?>][menu-item-description]" class="code menu-item-textbox" title="<?php esc_attr_e('Text/html/shortcode here!'); ?>">Callback=function_name&#13;&#10;key=value</textarea>
                </p>

                <p class="button-controls">
                    <span class="add-to-menu">
                        <input type="submit"<?php wp_nav_menu_disabled_check($nav_menu_selected_id); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e('Add to Menu'); ?>" name="add-gs-cim-menu-item" id="submit-gs-cim" />
                        <span class="spinner"></span>
                    </span>
                </p>
        ...
        public function start_el($item_output, $item) {
            ...
            switch (true){
                case (strpos($item->post_title, 'FULL HTML OUTPUT')):
                    //  legacy hack
                    ...
                    break;
                case ($item->object == 'gs_sim'):
                    // just process it
                    $shortcode_output = stripslashes(do_shortcode($item->description));
                    
                    $item_output ='<a href="'.$shortcode_output.'">'.$item->post_title;
                    if(!empty($item->post_excerpt)) $item_output =$item_output. '<span>'.$item->post_excerpt.'</span>';
                    $item_output = $item_output.'</a>';
                    break;
                case ($item->object == 'gs_cim'):
                    // just process it
                    $callback_request=array();
                    if(strstr($item->description, PHP_EOL)) $callback_request = explode(PHP_EOL,$item->description);
                    else $callback_request[] = $item->description;
                    
                    //let's try to get the callback function
                    if(strstr($callback_request[0], "=")){
                        $pair = explode("=",$callback_request[0]);
                        $callback = $pair[1];
                    }else $callback =$callback_request[0]; //let's try this
                    
                    //let's grab argument keys if any
                    $args=array();
                    for($idx=1;$idx<sizeof($callback_request);$idx++){
                        if(!strstr($callback_request[0], "=")) continue;
                        $pair = explode("=",$callback_request[$idx]);
                        $arg[$pair[0]]=$pair[1];
                    }
                    if (is_callable($callback)) {
                        $func = new ReflectionFunction($callback);
                        switch($func->getNumberOfParameters()){
                            case 4:
                                $item_output = call_user_func($callback,$item->post_title ,$item->post_excerpt,$item->target, $args );
                                break;
                            case 3:
                                $item_output = call_user_func($callback,$item->post_title ,$item->post_excerpt,$item->target );
                                break;
                            case 2:
                                $item_output = call_user_func($callback,$item->post_title ,$item->post_excerpt);
                                break;
                            case 1:
                                $item_output = call_user_func($callback,$item->post_title);
                                break;
                            default:
                                $item_output = call_user_func($callback);
                                break;
                        }
                        
                    }else $item_output = "Missing Function: ".$callback;
        ...
        public function setup_item($item) {
            ...
            switch(true){
                case ($item->object == 'gs_sim'): 
                    $foundObject=true;
                    // setup our label
                    $item->type_label = __('Shortcode (for full html use links)', 'gs_sim');
                    break;
                case ($item->object == 'gs_cim'): 
                    $foundObject=true;
                    // setup our label
                    $item->type_label = __('Callback function', 'gs_sim');
